...
def bandwidth(series,peak_wl,peak_loss,depth=1):			#返回带宽，默认为1dB带宽，由于光谱仪采点有限添加插值法取数
		# df=self.pure_loss()
		loss=series[series.index>=peak_wl-1]
		loss=loss[loss.index<=peak_wl+1]
		# print(loss)
		# peak_wl,peak_loss=self.find_peak_array()
		# print(peak_loss-depth)
		for index,los in enumerate(loss):
			if los<=peak_loss-depth:
				# print wl,los
				los_ll=los
		...
		wl_ll=loss.index[wl_ll]
		wl_lr=loss.index[wl_lr]
		loss_tmp=loss[loss.index>=peak_wl]
		# print loss_tmp
		for index,los in enumerate(loss_tmp):
			if los>=peak_loss-depth:
				los_rl=los
...
def region_interpolation(series,left,right):			#插值法取数，left,right为左右波长边界，返回值为left到right范围内的光谱数据
	...
	# df=self.pure_loss()
	# print(left,right)
	# print(series[series.index<=right+1])
...
def APR(file):                #返回分析后的测试数据，包括各通道中心波长及对应插损，通带范围，与其它各通道的串扰，及总中心波长，间隔
	...
	try:
		gap=abs((find_peak_array(file['CH 1'])[0]-find_peak_array(file['CH 70'])[0])/69)
	except:
		gap=abs((find_peak_array(file['CH 1'])[0]-find_peak_array(file['CH 16'])[0])/15)
	result=[]
	print(gap)
...
def channel_shift(acpl):
	result=[]
	for cols in acpl.columns:
		result.append(acpl[cols])
		# print(cols,find_peak_array(acpl[cols])[0])
	tmp=sorted(result,key=lambda x:find_peak_array(x)[0])
...
if __name__=='__main__':
	...
	path=input('path:')
	save_path='D:\\tmp\\test'	
	save_path=os.path.join(save_path,path.split('\\')[-1])

	files=filter(lambda x:x.endswith('ACPL.csv'),os.listdir(path))
	for txt_file in files:
		print(txt_file)
		acpl=pd.read_csv(path+'\\'+txt_file)
		acpl.columns=['WL']+['CH '+str(i) for i in range(1,17)]
		acpl.set_index('WL',inplace=True)
		acpl=acpl[acpl.index>1540]
		acpl=acpl[acpl.index<1560]
		acpl=channel_shift(acpl)
		APR(acpl).to_csv(path+'\\'+txt_file.split(' ')[0]+' APR.csv')

		#为0.4nm间隔设计
		# acpl_l=acpl[acpl.index<1545]
		# acpl_l=channel_shift(acpl_l)
		# apr_l=APR(acpl_l)

		# acpl_m=acpl[acpl.index>1545]
		# acpl_m=acpl_m[acpl_m.index<1560]
		# acpl_m=channel_shift(acpl_m)
		# apr_m=APR(acpl_m)

		# acpl_r=acpl[acpl.index>1560]
		# acpl_r=channel_shift(acpl_r)
		# apr_r=APR(acpl_r)

		# tmp=pd.concat([apr_l,apr_m,apr_r])
		# tmp.to_csv(path+'\\'+txt_file.split(' ')[0]+' APR.csv')
